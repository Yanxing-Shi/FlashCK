...
inline void check_sqlite(int result, sqlite3* cache_db, const char* file, const int line)
{
    ...
    if (result != SQLITE_OK && result != SQLITE_DONE && result != SQLITE_ROW) {
        ss << "[LI][ERROR][SQLITE] " << sqlite3_errmsg(cache_db) << ", " << "File: " << file << ", "
           << "Line: " << line << " \n";
...
#define LI_CHECK_SQLITE(val, cache_db) check_sqlite(val, cache_db, __FILE__, __LINE__)
...
inline DataType GetModelFileType(std::string ini_file, std::string section_name)
{
    DataType  model_file_type;
    INIReader reader = INIReader(ini_file);
    if (reader.ParseError() < 0) {
        LOG(WARNING) << "Can't load " << ini_file << ". Use FP32 as default";
        model_file_type = DataType::FLOAT32;
    }
    else {
        ...
        if (weight_data_type_str.find("fp32") != std::string::npos) {
            ...
            model_file_type = DataType::FLOAT16;
        }
        else if (weight_data_type_str.find("bf16") != std::string::npos) {
            model_file_type = DataType::BFLOAT16;
        }
        else {
            LOG(WARNING) << "Invalid type " << weight_data_type_str << ". Use FP32 as default";
            model_file_type = DataType::FLOAT32;
        }
    }
