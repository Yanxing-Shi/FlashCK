...
#include "lightinfer/core/module/kernels/gemm_kernels/bmm_epilogue_kernels/bmm_rcr_kernel.h"
...
std::map<std::string, std::vector<std::shared_ptr<DimInfo>>> BmmRCROp<CppType>::ExtractDimsImpl(bool for_profiling)
{
    ...
    batch_dim.push_back(std::make_shared<DimInfo>(TensorSource::Output, 0, output_shape.GetDim(0).GetValues()));
    ...
    return {{"B", batch_dim},
            {"M",
             {std::make_shared<DimInfo>(TensorSource::Input, 0, m_in_a),
              std::make_shared<DimInfo>(TensorSource::Output, 0, m_in_c)}},
            {"N",
             {std::make_shared<DimInfo>(TensorSource::Input, 1, n_in_b),
              std::make_shared<DimInfo>(TensorSource::Output, 0, n_in_c)}},
            {"K",
             {std::make_shared<DimInfo>(TensorSource::Input, 0, k_in_a),
              std::make_shared<DimInfo>(TensorSource::Input, 1, k_in_b)}}};
}
