...
#include "lightinfer/core/utils/debug_utils.h"
...
template<typename T>
void LayerNormOp<T>::CheckShape(const Shape& x_shape,
                                ...
                                const Shape& normalized_shape)
{
    ...
    CheckParamShape(x_shape, beta_shape, "beta");
...
template<typename T>
std::vector<Shape> LayerNormOp<T>::GetInputShape(Variable* x,
                                                 Variable* gamma,
                                                 Variable* beta,
                                                 Variable* x_bias,
                                                 Variable* x_residual,
                                                 ...
                                                 Variable* y_scale)
{
    ...
    Shape beta_shape  = beta == nullptr ? Shape() : beta->GetShape();

    Shape x_bias_shape       = x_bias == nullptr ? Shape() : x_bias->GetShape();
...
void LayerNormOp<T>::SanityCheck(Variable* x,
                                 ...
                                 Variable* y_scale)
{
    std::vector<Shape> input_shape =
        GetInputShape(x, gamma, beta, x_bias, x_residual, smooth_scale, y_residual, y_scale);
                                     ...
                                     const float eps)
{
    ...
    if (is_add_bias_ != NormBiasEnum::NO_BIAS && x_bias == nullptr) {
        LI_THROW(Unimplemented("bias add requires x_bias to be provided"));
    }
    if (fused_add_ != FusedAddEnum::NO_ADD && x_residual == nullptr) {
        LI_THROW(Unimplemented("Fused add requires x_residual to be provided"));
    }
...
template<typename T>
std::vector<std::string> LayerNormOp<T>::GenOpProfileCmd(const std::string&          profiler_prefix,
...
void LayerNormOp<T>::Forward()
{
    ...
    layer_norm_args.x_stride_  = x_stride_ == -1 ? layer_norm_args.x_dim_1_ : x_stride_;
    layer_norm_args.xr_stride_ = xr_stride_ == -1 ? layer_norm_args.x_dim_1_ : xr_stride_;
    layer_norm_args.y_stride_  = y_stride_ == -1 ? layer_norm_args.x_dim_1_ : y_stride_;
    layer_norm_args.yr_stride_ = yr_stride_ == -1 ? layer_norm_args.x_dim_1_ : yr_stride_;
