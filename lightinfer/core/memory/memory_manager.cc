...
{
    VLOG(1) << "Execute MemoryManager calculate_buffer";
    ...
    try {
        ...
        LI_THROW(ResourceExhausted("{}", "execute MemoryManager clear buffer failed!"));
    }
    ...
    for (int i = 0; i < ordered_tensor_usages.size(); i++) {
        ...
        if ((i + 1 == ordered_tensor_usages.size()) || (max_last_addr == ordered_tensor_usages[i + 1].second)) {
            ...
            try {
                ...
                LI_THROW(ResourceExhausted("allocate shared buffer failed!, buffer size is {}",
                                           buffer_vec_.size(),
                                           HumanReadableSize(max_last_addr - record_last_addr)));
            }

            VLOG(1) << "MemoryManager allocate success!";
