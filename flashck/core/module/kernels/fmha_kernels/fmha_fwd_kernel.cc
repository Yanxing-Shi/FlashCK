                                 ...
                                 const std::string&                               folder_name)
{
    ...
    jinja2::ValuesMap func_signature_value_map{{"function_name", "FmhaFwd"}, {"is_running", false}};
    std::string       func_signature = TemplateLoadAndRender(g_fmha_fwd_func_signature_tpl, func_signature_value_map);
    ...
    std::string       func_call = TemplateLoadAndRender(g_fmha_fwd_func_call_tpl, func_call_value_map);

    jinja2::ValuesMap prepare_args_value_map{{"mode_str", g_fmha_operation_mode_name_map.at(op_mode)},
                                             {"bias_str", g_bias_enum_names_map.at(bias_enum)},
                                             {"bias_rank_info", bias_rank_info}};
    std::string       prepare_args = TemplateLoadAndRender(g_fmha_fwd_prepare_args_tpl, prepare_args_value_map);

    jinja2::ValuesMap make_args_value_map{{"mode_str", g_fmha_operation_mode_name_map.at(op_mode)},
                                          {"kernel_name", "FmhaInstance"}};
    std::string       make_args = TemplateLoadAndRender(g_fmha_fwd_make_args_tpl, make_args_value_map);

    jinja2::ValuesMap tensor_decl_value_map{{"bias_str", g_bias_enum_names_map.at(bias_enum)},
                                            {"bias_rank_info", bias_rank_info}};
    std::string       tensor_decl = TemplateLoadAndRender(g_fmha_fwd_tensor_decl_tpl, tensor_decl_value_map);
    ...
    jinja2::ValuesMap tensor_generate_value_map{{"init_method_str", "uf"},
                                                ...
                                                {"bias_rank_info", bias_rank_info},
                                                {"seed", 12456}};
    std::string tenosr_generate = TemplateLoadAndRender(g_fmha_fwd_tensor_generate_tpl, tensor_generate_value_map);

    return GenFmhaCommonKernelProfiler(model_name,
                                       kernel_func_map,
                                       g_fmha_fwd_create_args_tpl,
