...
void LayerNormOp<T>::ExtractExecPath(const ProfilingStrategy& profiling_strategy, const int step_value)
{
    ...
    if (profiling_strategy == ProfilingStrategy::kMax) {
        ...
        exec_paths_.push_back(RunningItem{
            .profiling_key_ = GenWorkLoad(max_values),
            .exec_cond_     = GenWorkLoad(shape_values_map),
            .instance_name_ = "",
        ...
        exec_paths_.push_back(RunningItem{
            .profiling_key_ = GenWorkLoad(min_values),
            .exec_cond_     = GenWorkLoad(shape_values_map),
            .instance_name_ = "",
            .perf_result_   = PerfResult(),
        });
        ...
        for (const auto& iter_value : iter_values_vec) {
            exec_paths_.push_back(RunningItem{
                .profiling_key_ = GenWorkLoad(iter_value),
                .exec_cond_     = GenWorkLoad(iter_value),
                .instance_name_ = "",
                .perf_result_   = PerfResult(),
            });
        }
...
template<typename T>
void LayerNormOp<T>::IsBuildProfilingEngine(const std::vector<std::string>& workloads)
{
    for (const auto& workload : exec_paths_) {
        std::map<std::string, int> profiling_key_map = ExtractWorkLoad(workload.profiling_key_);
        auto                       instance_data     = InstanceData{
            Environment{.environment_ = GetDeviceName(),
                                                  .rocm_version_ = ProfilingEngine::Instance()->GetCompiler()->GetROCMVersion()},
            Setting{},
            CodeGenKind::Norm,
            NormProblem{DataType::CppTypeToDataType<T>::Type(),
                        DataType::CppTypeToDataType<T>::Type(),
                        DataType::FLOAT32,
                        DataType::FLOAT32,
                        profiling_key_map.at("m").front(),
                        profiling_key_map.at("n").front(),
                        op_kind_,
                        epilogue_op_,
                        is_add_bias_,
                        fused_add_,
    ...
    if (instance_name != "null" && perf_result != -1 && !FLAGS_FC_FORCE_PROFILE) {
        LOG(INFO) << "Load profiling result for layer norm from database, " << instance_name
                  ...
                  << "tflops: " << perf_result.tflops_ << "GB, " << "throughput: " << perf_result.throughput_ << "GB/s";
...
LayerNormOp<T>::GenOpProfiler(const ProfilingStrategy& profiling_strategy)
{
    ...
    if (!FLAGS_FC_FORCE_PROFILER_CACHE) {
        IsBuildProfilingEngine(profiling_key_);
    }
    else {
    ...
    // generate profiling problems
    std::vector<NormProblem> layernorm_problems;
    for (const auto& workload : exec_paths_) {
        std::map<std::string, int> profiling_key_map = ExtractWorkLoad(workload.profiling_key_);
        layernorm_problems.push_back(NormProblem{
            DataType::CppTypeToDataType<T>::Type(),
            DataType::CppTypeToDataType<T>::Type(),
            DataType::FLOAT32,
            DataType::FLOAT32,
    ...
    for (int i = 0; i < profiling_key_.size(); i++) {
        ProfileEngine::GetInstance()->GenerateInstances(CodeGenKind::Norm, layernorm_problems[i]);

        auto norm_instance_map = ProfileEngine::GetInstance()->norm_instance_map_;
        if (norm_instance_map.size() == 0) {
            FC_THROW(Fatal("No layernorm op instances were generated for {}", op_name_));
        }

        if (exec_paths_[profiling_key_[i]]->algo_ == "") {
