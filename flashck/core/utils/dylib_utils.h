...
public:
    ...
    explicit dylib(const char* lib_name, bool decorations = add_filename_decorations): dylib("", lib_name, decorations)
    {
    }

#ifdef DYLIB_CPP17
    explicit dylib(const std::filesystem::path& lib_path): dylib("", lib_path.string().c_str(), no_filename_decorations)
    {
    }

    dylib(const std::filesystem::path& dir_path,
...
#pragma GCC diagnostic pop
    ...
    template<typename T>
    T get2_function(const char* symbol_name) const
    {
#if (defined(__GNUC__) && __GNUC__ >= 8)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wcast-function-type"
...
#pragma GCC diagnostic pop
    ...
    template<typename T>
    T get2_function(const std::string& symbol_name) const
    {
        return get2_function<T>(symbol_name.c_str());
    }
    /**
