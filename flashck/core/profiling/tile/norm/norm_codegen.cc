#include "flashck/core/profiling/codegen/tile/norm/norm_codegen.h"
...
std::string NormCodeGen::GetConfigName()
{
    return Sprintf("{op_kind_name}_{x_dtype}_{y_dtype}_{smooth_scale_dtype}_{y_scale_dtype}_"
                   "{tile_desc}_{is_add_bias}_{fused_add}_{fused_quant}",
                   fmt::arg("op_kind_name", norm_map.at(kind_).short_name),
                   fmt::arg("x_dtype", DataTypeoString(x_dtype_)),
                   ...
                   fmt::arg("is_add_bias", norm_bias_map.at(is_add_bias_).short_name),
                   fmt::arg("fused_add", fused_add_map.at(fused_add_).short_name),
                   fmt::arg("fused_quant", fused_quant_map.at(fused_quant_).short_name));
}

std::string NormCodeGen::Emit()
{
    std::string source = R"(
...
{% endif %}
    ...
    jinja2::ValuesMap value_map{{"name", GetConfigName()},
                                {"idx", idx++},
                                {"norm_kind", norm_map.at(kind_).name},
                                {"norm_problem", norm_map.at(kind_).problem_tag},
                                {"norm_traits", norm_map.at(kind_).trait_tag},
                                {"norm_fwd", norm_map.at(kind_).fwd_tag},
                                {"norm_pass", norm_map.at(kind_).pass_tag},
                                {"is_pad_n", "true"},
                                {"is_fast_div", "true"},
                                {"is_two_pass", "true"},
                                {"is_welford", "true"},
