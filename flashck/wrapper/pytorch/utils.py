...
def _try_import_flashck_functions():
    ...
    try:
        from flash_ck_torch import layer_norm_fwd, layer_norm_fwd_dynamic
        _FLASHCK_FUNCTIONS[FlashCKOperationType.LAYER_NORM.value] = {
            'forward': layer_norm_fwd,
            'forward_dynamic': layer_norm_fwd_dynamic,
        }
    ...
    try:
        # from flash_ck_torch import fmha_fwd, fmha_bwd
...
def is_flashck_available(operation_type: Optional[str] = None) -> bool:
    """
...
def get_flashck_functions(operation_type: str) -> Optional[Dict[str, Callable]]:
    ...
    """
    if not is_flashck_available(operation_type):
        return None
