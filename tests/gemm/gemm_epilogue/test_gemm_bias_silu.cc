...
template<typename T>
class GemmBiasSiluTest: public TestBase {
public:
    void SetUp() override {};
    ...
    void BuildTorchModel(bool is_benchmark)
    {
        auto torch_func = [&](torch::Tensor x_pt, torch::Tensor w_pt, torch::Tensor b_pt) {
            return torch::nn::functional::silu(torch::nn::functional::linear(x_pt, w_pt, b_pt)).to(torch::kFloat32);
        };
        ...
        y_silu_out_pt_ = torch_func(x_pt_, w_pt_, b_pt_);
    }
                              ...
                              const std::string&          test_name)
    {
        ...
        auto linear_layer = std::make_unique<lightinfer::LinearLayer<T>>(k_runtime, n_runtime, false, true, "silu");
    ...
    void RunTestGemmBiasSilu(const std::vector<int64_t>& m_max,
                             const std::vector<int64_t>& m_runtime,
                             const int64_t               n_runtime,
                             const int64_t               k_runtime,
                             bool                        is_benchmark,
                             const std::string&          test_name)
    {
        ...
        if (m_max.size() != m_runtime.size()) {
            throw std::runtime_error("RunTestGemmBiasSilu: invalid input shape");
        }
        ...
        bool passed = CheckResult(test_name,
                                  ...
                                  y_silu_out_pt_.data_ptr<float>(),
                                  y_silu_out_pt_.numel());
        EXPECT_TRUE(passed);
    }
...
private:
    ...
    torch::Tensor         y_silu_out_pt_;
    lightinfer::Variable* y_out_ater_ = nullptr;

    int test_id_ = 0;
};

TYPED_TEST_SUITE(GemmBiasSiluTest, KernelTestTypes);

TYPED_TEST(GemmBiasSiluTest, test_gemm_bias_silu_2d_static)
{
    this->RunTestGemmBiasSilu({1024}, {1024}, 256, 512, true, "test_gemm_bias_silu_2d_static");
}

TYPED_TEST(GemmBiasSiluTest, test_gemm_bias_silu_3d_static)
{
    this->RunTestGemmBiasSilu({2, 1024}, {2, 1024}, 256, 512, true, "test_gemm_bias_silu_3d_static");
}