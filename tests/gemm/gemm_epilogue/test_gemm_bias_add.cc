...
template<typename T>
class GemmBiasAddTest: public TestBase {
public:
    void SetUp() override {};
    ...
    {
        auto torch_func = [&](torch::Tensor x, torch::Tensor w, torch::Tensor b, torch::Tensor d0) {
            return torch::add(torch::nn::functional::linear(x, w, b), d0).to(torch::kFloat32);
        };
        ...
        y_add_out_pt_ = torch_func(x_pt_, w_pt_, b_pt_, d0_pt_);
    }
                              ...
                              const std::string&          test_name)
    {
        ...
        auto d0 = std::make_unique<lightinfer::Variable>("d0_var", lightinfer::CppTypeToDataType<T>::Type());
        if (m_max.size() == 1) {
            x->SetShape({lightinfer::DDim({1, m_max[0]}), lightinfer::DDim(k_runtime)});
            d0->SetShape({lightinfer::DDim({1, m_max[0]}), lightinfer::DDim(n_runtime)});
        }
        else {
            x->SetShape(
                {lightinfer::DDim({1, m_max[0]}), lightinfer::DDim({1, m_max[1]}), lightinfer::DDim(k_runtime)});
            d0->SetShape(
                {lightinfer::DDim({1, m_max[0]}), lightinfer::DDim({1, m_max[1]}), lightinfer::DDim(n_runtime)});
        }
        auto linear_layer = std::make_unique<lightinfer::LinearLayer<T>>(k_runtime, n_runtime, false, true, "add");
    ...
    void RunTestGemmBiasAdd(const std::vector<int64_t>& m_max,
                            const std::vector<int64_t>& m_runtime,
                            const int64_t               n_runtime,
                            const int64_t               k_runtime,
                            bool                        is_benchmark,
...
private:
    ...
    torch::Tensor         y_add_out_pt_;
    lightinfer::Variable* y_out_ater_ = nullptr;

    int test_id_ = 0;
};

TYPED_TEST_SUITE(GemmBiasAddTest, KernelTestTypes);

TYPED_TEST(GemmBiasAddTest, test_gemm_bias_add_2d_static)
{
    this->RunTestGemmBiasAdd({1024}, {1024}, 256, 512, true, "test_gemm_bias_add_2d_static");
}

TYPED_TEST(GemmBiasAddTest, test_gemm_bias_add_3d_static)
{
    this->RunTestGemmBiasAdd({2, 1024}, {2, 1024}, 256, 512, true, "test_gemm_bias_add_3d_static");
}