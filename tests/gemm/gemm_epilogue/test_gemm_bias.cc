...
template<typename T>
class GemmBiasTest: public TestBase {
public:
    void SetUp() override {};
                              ...
                              const std::string&          test_name)
    {
        ...
        auto linear_layer = std::make_unique<lightinfer::LinearLayer<T>>(k_runtime, n_runtime, false, true);
    ...
    void RunTestGemmBias(const std::vector<int64_t>& m_max,
                         const std::vector<int64_t>& m_runtime,
                         const int64_t               n_runtime,
                         const int64_t               k_runtime,
                         bool                        is_benchmark,
                         const std::string&          test_name)
    {
        ...
        if (m_max.size() != m_runtime.size()) {
            throw std::runtime_error("RunTestGemmBias: invalid input shape");
        }
...
TYPED_TEST_SUITE(GemmBiasTest, KernelTestTypes);

TYPED_TEST(GemmBiasTest, test_gemm_bias_2d_static)
{
    this->RunTestGemmBias({1024}, {1024}, 256, 512, true, "test_gemm_bias_2d_static");
}

TYPED_TEST(GemmBiasTest, test_gemm_bias_3d_static)
{
    this->RunTestGemmBias({1, 1024}, {1, 1024}, 256, 512, true, "test_gemm_bias_3d_static");
}