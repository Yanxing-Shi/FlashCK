...
public:
                            ...
                            const std::string&          test_name)
    {
        ...
        x->SetShape({lightinfer::DDim({1, m_max[0]}), lightinfer::DDim({1, m_max[1]}), lightinfer::DDim(k_runtime)});

        auto linear_layer = std::make_unique<lightinfer::LinearLayer<T>>(
            k_runtime, n_mul_res_, false, true, "permute", lightinfer::Shape({n_runtime[0], n_runtime[1]}));
