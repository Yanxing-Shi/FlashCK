...
public:
    ...
    void BuildLightInferModel(const int64_t      b_max,
                              const int64_t      m_max,
                              ...
                              const std::string& test_name)
    {
        ...
        x->SetShape({lightinfer::DDim({1, b_max}), lightinfer::DDim({1, m_max}), lightinfer::DDim(k_runtime)});

        auto bmm_layer = std::make_unique<lightinfer::BmmLayer<T>>(k_runtime, n_runtime, false);
        ...
        if (is_benchmark) {
            benchmark_.BenchmarkLightInferFunc(bmm_layer.get());
        }
                     ...
                     const std::string& test_name)
    {
        ...
        BuildLightInferModel(b_max, m_max, b_runtime, m_runtime, n_runtime, k_runtime, is_benchmark, test_name);
