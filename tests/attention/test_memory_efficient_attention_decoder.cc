...
#include "lightinfer/core/module/layers/attention_layers/memory_efficient_attention_decoder_layer.h"
...
public:
    ...
    void BuildLightInferModel(const DecoderAttentionConfigs& configs,
                              int64_t                        rotary_dim,
                              ...
                              const std::string&             test_name    = "")
    {
        ...
        auto ceildiv_ddim = [](lightinfer::DDim a, lightinfer::DDim b) { return (a + b - lightinfer::DDim(1)) / b; };
        lightinfer::DDim max_num_page_blocks_dim =
            lightinfer::DDim({1, configs.batch_size_})
            * ceildiv_ddim(lightinfer::DDim({1, configs.kv_seq_len_}), configs.paged_block_size_);

        auto q_var = std::make_unique<lightinfer::Variable>("q_var", lightinfer::CppTypeToDataType<T>::Type());
        auto cache_k_var =
            std::make_unique<lightinfer::Variable>("cache_k_var", lightinfer::CppTypeToDataType<T>::Type());
        auto cache_v_var =
            std::make_unique<lightinfer::Variable>("cache_v_var", lightinfer::CppTypeToDataType<T>::Type());
        ...
        auto rotary_cos_var =
            rotary_dim > 0 ?
                std::make_unique<lightinfer::Variable>("rotary_cos_var", lightinfer::CppTypeToDataType<T>::Type()) :
                nullptr;
        auto rotary_sin_var =
            rotary_dim > 0 ?
                std::make_unique<lightinfer::Variable>("rotary_sin_var", lightinfer::CppTypeToDataType<T>::Type()) :
                nullptr;
        auto cache_batch_idx_var =
            configs.use_batch_cache_idx_ ?
                std::make_unique<lightinfer::Variable>("cache_batch_idx_var", lightinfer::DataType::INT64) :
                nullptr;
        auto block_table_var =
            configs.paged_block_size_ > 0 ?
                std::make_unique<lightinfer::Variable>("block_table_var", lightinfer::CppTypeToDataType<T>::Type()) :
                nullptr;
        auto cache_seqlen_k_var =
            std::make_unique<lightinfer::Variable>("cache_seqlen_k_var", lightinfer::DataType::INT64);
        ...
        if (configs.paged_block_size_ > 0) {
            cache_k_var->SetShape({
                ...
                lightinfer::DDim(configs.paged_block_size_),
        ...
        k_var->SetShape({
            lightinfer::DDim({1, configs.batch_size_}),
            lightinfer::DDim({1, configs.new_kv_seq_len_}),
            lightinfer::DDim(configs.kv_num_heads_),
            lightinfer::DDim(configs.qk_head_dim_),
        });
        v_var->SetShape({
        ...
        if (rotary_dim > 0) {
            rotary_cos_var->SetShape({
                lightinfer::DDim({2, std::max(configs.q_seq_len_, configs.kv_seq_len_) * 2}),
                lightinfer::DDim(rotary_dim / 2),
            });
        ...
        if (configs.use_batch_cache_idx_) {
            cache_batch_idx_var->SetShape({lightinfer::DDim({1, batch_size_cache})});
        }

        if (configs.paged_block_size_ > 0) {
            block_table_var->SetShape({lightinfer::DDim({1, configs.batch_size_}),
                                       max_num_page_blocks_dim / lightinfer::DDim({1, configs.batch_size_})});
        }

        cache_seqlen_k_var->SetShape({lightinfer::DDim({1, configs.batch_size_})});

        auto attn_layer =
            std::make_unique<lightinfer::MemoryEfficientAttentionDecoderLayer<T>>(lightinfer::FmhaOperationMode::Batch,
                                                ...
                                                const std::string&             test_name    = "")
    {
                                    ...
                                    (configs.new_kv_seq_len_ > 0 ?
                                         (configs.kv_seq_len_
                                          - ((configs.window_size_ != std::array<int64_t, 2>({-1, -1})
                                              || configs.mask_enum_ != lightinfer::GenericAttentionMaskEnum::NO_MASK)
                                                     && rotary_dim > 1 ?
                                                 configs.q_seq_len_ :
                                                 configs.new_kv_seq_len_)
                                          + 1) :
        ...
        std::tie(sin_, cos_, q_rotated, k_rotated) = CreateRotaryEmbeddingTensor<T>(
            q_, k_, rotary_dim, configs.rope_enum_ == lightinfer::RopeEnum::HALF_ROTATED, kv_cache_seqlen_expanded);
        ...
        BuildLightInferModel(configs, rotary_dim, batch_size_cache, is_benchmark, test_name);
    ...
    this->RunTestMemoryEfficientAttentionDecoder(
        {
            ...
            4096,
            0.5f,
            lightinfer::RopeEnum::HALF_ROTATED,
            4,
        },
        false,
