...
inline torch::Tensor GetSlopesTensor(const int64_t               num_heads,
                                     ...
                                     const lightinfer::BiasEnum& bias_enum,
...
)
{
    ...
    if (bias_enum == lightinfer::BiasEnum::NO_BIAS) {
        return std::make_tuple(bias_tensor, alibi_slopes_tensor);
    }
    else if (bias_enum == lightinfer::BiasEnum::ELEMENTWISE_BIAS) {
        if (bias_rank_info == 0) {
    ...
    else if (bias_enum == lightinfer::BiasEnum::ALIBI) {
        if (bias_rank_info == 0) {
            // [1, q_num_heads]
