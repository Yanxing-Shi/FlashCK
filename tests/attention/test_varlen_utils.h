...
)
{
    ...
    if (configs.is_qkv_packed_) {
        if (!(query_padding_mask == key_padding_mask).all()) {
            LI_THROW(InvalidArgument("query_padding_mask and key_padding_mask should be the same"));
        }
        LI_ENFORCE_EQ(q_num_heads, kv_num_heads, Unimplemented("QKV packed Only support same num_heads"));
