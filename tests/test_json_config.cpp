...
int main() {
    ...
    try {
        auto single_config = LoadDefaultConfigJson<FmhaBatchPrefillConfig>("configs/fmha/fmha_batch_prefill/default_config.json");
        std::cout << "Single config loaded successfully!" << std::endl;
        std::cout << "Pipeline values count: " << single_config.pipeline.values.size() << std::endl;
        std::cout << "Block tile m0 values count: " << single_config.tile_shape.block_tile.m0.values.size() << std::endl;
    ...
    try {
        auto backup_configs = LoadBackupConfigJson<FmhaBatchPrefillConfig>("configs/fmha/fmha_batch_prefill/backup_config.json");
        ...
        for (size_t i = 0; i < backup_configs.size(); ++i) {
            ...
            std::cout << "  - Block tile m0: " << config.tile_shape.block_tile.m0.values[0] << std::endl;
            std::cout << "  - Block tile n0: " << config.tile_shape.block_tile.n0.values[0] << std::endl;
            std::cout << "  - Block tile k0: " << config.tile_shape.block_tile.k0.values[0] << std::endl;
            std::cout << "  - Padding s: " << (config.padding.s.values[0] ? "true" : "false") << std::endl;
            ...
            assert(config.tile_shape.block_tile.k0.values.size() == 1);
            assert(config.padding.s.values.size() == 1);
