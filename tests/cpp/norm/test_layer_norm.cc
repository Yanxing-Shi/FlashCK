...
TEST_F(NormUnifiedTestFloat, LayerNormCorrectnessTest)
{
    ...
    auto reference_impl = [](const NormConfig<float>& config, float* output) {
        LayerNormReference<float>::forward(config, output);
    };
    auto flashck_impl = [](const NormConfig<float>& config, GpuMemoryManager<float>& gpu_mem) -> float* {
        try {
    ...
    run_correctness_test(configs,
                         std::function<void(const NormConfig<float>&, float*)>(reference_impl),
                         std::function<float*(const NormConfig<float>&, GpuMemoryManager<float>&)>(flashck_impl),
                         1e-3f,
                         1e-4f,
                         true);
}
...
{
    std::vector<std::shared_ptr<NormConfig<float>>> perf_configs;
    perf_configs.push_back(std::make_shared<NormConfig<float>>(64, 512, 1e-5f, "Small_64x512"));
    perf_configs.push_back(std::make_shared<NormConfig<float>>(128, 768, 1e-5f, "Medium_128x768"));
    perf_configs.push_back(std::make_shared<NormConfig<float>>(256, 1024, 1e-5f, "Large_256x1024"));
    perf_configs.push_back(std::make_shared<NormConfig<float>>(512, 2048, 1e-5f, "XLarge_512x2048"));
    perf_configs.push_back(std::make_shared<NormConfig<float>>(1024, 4096, 1e-5f, "XXLarge_1024x4096"));
    auto flashck_impl = [](const NormConfig<float>& config, GpuMemoryManager<float>& gpu_mem) -> float* {
        try {
            return layer_norm_fwd(
                config.gpu_input(), config.gpu_gamma(), config.gpu_beta(), config.m(), config.n(), config.epsilon());
