#include "core/module/operations/norm_ops/layer_norm_op.h"
...
void LayerNormOp<T>::ExtractRunningInfo(const ProfilingStrategy& profiling_strategy, const int step_value)
{
    ...
    if (profiling_strategy == ProfilingStrategy::kMax) {
        ...
        FC_THROW(Unimplemented("norm only supports MIN, MAX or ITERATION profiling strategies"));
    }
}
...
LayerNormOp<T>::CodeGenForTuning(const ProfilingStrategy& profiling_strategy)
{
    ...
    for (const auto& [workload_key, running_info] : running_infos_) {
        ...
        FC_ENFORCE_NE(instance_map_.size(), 0, Fatal("No LayerNorm instances generated"));

        if (running_info.IsInstanceExist()) {
            VLOG(1) << "LayerNorm instance already exists, skipping profiling";
            continue;
        }
                                          ...
                                          GPUProfilingRunner& profiling_runner)
{
    ...
    for (auto& [instance_name, _] : instance_map_) {
        ...
        VLOG(1) << "LayerNorm tuning command: " << JoinStrings(command, " ");
...
void LayerNormOp<T>::Tuning(GPUProfilingRunner& profiling_runner, const std::string& folder_name)
{
    ...
    for (const auto& [profiling_workload, running_item] : running_infos_) {
        if (!running_item.IsInstanceExist()) {
            ...
            VLOG(1) << "LayerNorm instance already exists, skipping tuning";
        }
    }
}
...
void LayerNormOp<T>::Forward()
{
    ...
    T*        beta_ptr  = reinterpret_cast<T*>(GetParentNode(2)->GetValue());

    // Optional tensors based on configuration flags
    int offset = 3;
    T*  x_bias_ptr =
        (is_add_bias_ != NormBiasEnum::NO_BIAS) ? reinterpret_cast<T*>(GetParentNode(offset++)->GetValue()) : nullptr;
    ...
    VLOG(1) << "LayerNorm forward: " << c_shape.ToString();

    // Debug tensor values (only in debug builds)
    if (VLOG_IS_ON(3)) {
        const std::string prefix = "[" + GetNormKindName(op_kind_) + "] ";
        PrintToScreen(x_ptr, 3, prefix + "x_ptr");
        PrintToScreen(gamma_ptr, 3, prefix + "gamma_ptr");
        PrintToScreen(beta_ptr, 3, prefix + "beta_ptr");
        PrintToScreen(x_bias_ptr, 3, prefix + "x_bias_ptr");
    ...
    kernel_args.x_bias_ptr_       = x_bias_ptr;
