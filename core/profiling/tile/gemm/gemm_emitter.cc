...
void GemmEmitter::GenerateInstances(GemmProblem& gemm_problem)
{
    ...
    if (FLAGS_FC_ENABLE_CONFIG_JSON) {
        ...
        if(FLAGS_FC_ENABLE_JSON_MODE == 0) {
            ...
            gemm_instances = GenerateTileGemmInstances(config, gemm_problem);
            ...
            auto gemm_default_instances = GenerateTileGemmInstances(default_config, gemm_problem);
            ...
            auto gemm_user_instances = GenerateTileGemmInstances(user_config, gemm_problem);
    ...
    for (const auto& config : g_tile_gemm_backup_tile_config) {
        ...
        gemm.tile_desc_ = GemmTileDesc{
            ...
            config.tile_config_.block_.n_.values_[0][0],
            config.tile_config_.block_.k_.values_[0][0],
            config.tile_config_.warp_.m_.values_[0][0],
            config.tile_config_.warp_.n_.values_[0][0],
            config.tile_config_.warp_.k_.values_[0][0],
            config.tile_config_.warp_tile_.m_.values_[0][0],
            config.tile_config_.warp_tile_.n_.values_[0][0],
            config.tile_config_.warp_tile_.k_.values_[0][0],
        };
