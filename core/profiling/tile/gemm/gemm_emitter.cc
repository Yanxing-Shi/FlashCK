#include "core/profiling/tile/gemm/gemm_emitter.h"
...
{
    return IsValidTile(instance.tile_desc_, instance.problem_) && IsValidCombination(instance.pipeline_version_, instance.epilogue_, instance.pipeline_scheduler_);
}
...
std::vector<GemmCodeGen> GemmEmitter::CreateInstanceForConfig(const flashck::TileGemmConfig& config, const GemmProblem& gemm_problem) {
    ...
    std::vector<std::vector<ProductElem>> all_lists = {
        // BlockConfig
        [&]{ std::vector<ProductElem> v; for (auto x : flatten(config.tile.block.m.values)) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        [&]{ std::vector<ProductElem> v; for (auto x : flatten(config.tile.block.n.values)) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        [&]{ std::vector<ProductElem> v; for (auto x : flatten(config.tile.block.k.values)) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        // WarpConfig
        [&]{ std::vector<ProductElem> v; for (auto x : flatten(config.tile.warp.m.values)) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        [&]{ std::vector<ProductElem> v; for (auto x : flatten(config.tile.warp.n.values)) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        [&]{ std::vector<ProductElem> v; for (auto x : flatten(config.tile.warp.k.values)) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        // WarpTileConfig
        [&]{ std::vector<ProductElem> v; for (auto x : flatten(config.tile.warp_tile.m.values)) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        [&]{ std::vector<ProductElem> v; for (auto x : flatten(config.tile.warp_tile.n.values)) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        [&]{ std::vector<ProductElem> v; for (auto x : flatten(config.tile.warp_tile.k.values)) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        // PaddingConfig (convert bool to int64_t)
        [&]{ std::vector<ProductElem> v; for (auto x : flatten(config.padding.m.values)) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        [&]{ std::vector<ProductElem> v; for (auto x : flatten(config.padding.n.values)) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        [&]{ std::vector<ProductElem> v; for (auto x : flatten(config.padding.k.values)) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        // LaunchConfig
        ...
        // PartitionConfig
        [&]{ std::vector<ProductElem> v; for (auto x : flatten(config.partition.num_wave_groups.values)) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        [&]{ std::vector<ProductElem> v; for (auto x : flatten(config.partition.tile_partitioner_group_num.values)) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        [&]{ std::vector<ProductElem> v; for (auto x : flatten(config.partition.tile_partitioner_m01.values)) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        // PipelineConfig (enum as int64_t)
        [&]{ std::vector<ProductElem> v; for (const auto& x : flatten(config.pipeline.version.values)) v.emplace_back(static_cast<int64_t>(GetPipelineVersionEnumFromString(x))); return v; }(),
        [&]{ std::vector<ProductElem> v; for (const auto& x : flatten(config.pipeline.scheduler.values)) v.emplace_back(static_cast<int64_t>(GetPipelineSchedulerEnumFromString(x))); return v; }(),
        // EpilogueConfig (enum as int64_t)
        [&]{ std::vector<ProductElem> v; for (const auto& x : flatten(config.epilogue.values)) v.emplace_back(static_cast<int64_t>(GetEpilogueEnumFromString(x))); return v; }(),
    };
...
void GemmEmitter::GenerateInstances(GemmProblem& gemm_problem)
{
    ...
    if (FLAGS_FC_ENABLE_CONFIG_JSON) {
        ...
        if(FLAGS_FC_ENABLE_JSON_MODE == 0) {
            ...
            TileGemmConfig config = LoadConfigJson<TileGemmConfig>(json_path);
            ...
            TileGemmConfig default_config = LoadConfigJson<TileGemmConfig>(default_json_path);
            ...
            TileGemmConfig user_config = LoadConfigJson<TileGemmConfig>(user_json_path);
    ...
    for (const auto& config : g_tile_gemm_backup_tile_config) {
        ...
        gemm.tile_desc_ = GemmTileDesc{
            config.tile.block.m.values[0][0],
            config.tile.block.n.values[0][0],
            config.tile.block.k.values[0][0],
            config.tile.warp.m.values[0][0],
            config.tile.warp.n.values[0][0],
            config.tile.warp.k.values[0][0],
            config.tile.warp_tile.m.values[0][0],
            config.tile.warp_tile.n.values[0][0],
            config.tile.warp_tile.k.values[0][0],
        };
        ...
        gemm.epilogue_ = GetEpilogueEnumFromString(config.epilogue.values[0]);

        gemm.min_block_per_cu_ = config.launch.min_block_per_cu.values[0][0];
        gemm.num_wave_groups_ = config.partition.num_wave_groups.values[0][0];
        gemm.tile_partitioner_group_num_ = config.partition.tile_partitioner_group_num.values[0][0];
        gemm.tile_partitioner_m01_ = config.partition.tile_partitioner_m01.values[0][0];
