...
bool LayerNormEmitter::IsValidTile(const LayerNormTileDesc& tile_desc, const NormProblem& norm_problem)
{
    ...
    if (tile_desc.n_vector % (4 / data_type_size) != 0) {
        VLOG(3) << "Invalid Layer Normalization tile: vector size " << tile_desc.n_vector 
...
bool LayerNormEmitter::IsValidInstance(const NormCodeGen& instance)
{
    return IsValidTile(instance.tile_desc_, instance.problem_);
}

std::vector<NormCodeGen> LayerNormEmitter::HeuristicFilter(
...
std::vector<NormCodeGen> LayerNormEmitter::CreateInstanceForConfig(
    const LayerNormConfig& config, const NormProblem& norm_problem) 
{
    ...
    std::vector<std::vector<int64_t>> all_param_lists = {
        ...
        [&]{ std::vector<int64_t> v; 
             for (auto x : config.thread_config.m_repeat.values) v.push_back(static_cast<int64_t>(x)); 
             return v; }(),
        [&]{ std::vector<int64_t> v; 
             for (auto x : config.thread_config.n_repeat.values) v.push_back(static_cast<int64_t>(x)); 
             return v; }(),
        [&]{ std::vector<int64_t> v; 
             for (auto x : config.thread_config.m_thread_per_block.values) v.push_back(static_cast<int64_t>(x)); 
             return v; }(),
        [&]{ std::vector<int64_t> v; 
             for (auto x : config.thread_config.n_thread_per_block.values) v.push_back(static_cast<int64_t>(x)); 
             return v; }(),
        [&]{ std::vector<int64_t> v; 
             for (auto x : config.thread_config.n_vector.values) v.push_back(static_cast<int64_t>(x)); 
             return v; }(),
    ...
    CartesianProduct(all_param_lists, [&](const std::vector<int64_t>& vals) {
        ...
        // Construct NormCodeGen for Layer Normalization
...
void LayerNormEmitter::GenerateInstances(NormProblem& norm_problem)
{
    ...
    if (instance_map_.find(norm_problem.kind_) != instance_map_.end() && 
        ...
        VLOG(2) << "Layer Normalization instances already generated for kind: " << GetNormKindName(norm_problem.kind_);
        return;
    }
    ...
    auto base_json_path = std::filesystem::path(FLAGS_FC_CONFIG_JSON_PATH) / "layer_norm";
    ...
    if (FLAGS_FC_ENABLE_BACKUP_JSON) {
        try {
            ...
            if (std::filesystem::exists(backup_path)) {
                auto backup_configs = LoadConfigJson<std::vector<LayerNormConfig>>(backup_path);
    ...
    if (FLAGS_FC_ENABLE_DEFAULT_JSON) {
        try {
            ...
            if (std::filesystem::exists(default_path)) {
                auto default_config = LoadConfigJson<LayerNormConfig>(default_path);
    ...
    if (FLAGS_FC_ENABLE_USER_JSON) {
        try {
            ...
            if (std::filesystem::exists(user_path)) {
                auto user_config = LoadConfigJson<LayerNormConfig>(user_path);
    ...
    VLOG(1) << "Generated " << num_instances_ << " valid Layer Normalization instances for " 
            << GetNormKindName(norm_problem.kind_) << " (mode " << FLAGS_FC_TUNING_MODE << ")";
}

int64_t LayerNormEmitter::GetNumInstances() const
{
