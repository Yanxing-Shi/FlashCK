...
void FmhaFwdSplitKVEmitter::GenerateInstances(FmhaProblem& fmha_problem)
{
    ...
    if (instance_map_.find(fmha_problem.kind_) != instance_map_.end() && 
        ...
        VLOG(2) << "Split-KV instances already generated for FMHA kind: " << GetFmhaKindName(fmha_problem.kind_);
        return;
    }
    ...
    VLOG(1) << "Generated " << generated_count << " FMHA split-KV instances for kind: " 
