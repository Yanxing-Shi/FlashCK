...
std::vector<FmhaFwdAppendKVCodeGen> FmhaFwdAppendKVEmitter::CreateInstanceForConfig(const FmhaFwdAppendKVConfig& config, const FmhaProblem& fmha_problem) {
    ...
    std::vector<std::vector<int64_t>> all_lists = {
        // BlockConfig
        [&]{ std::vector<int64_t> v; for (auto x : config.tile_shape.block_tile.s.values) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        [&]{ std::vector<int64_t> v; for (auto x : config.tile_shape.block_tile.sk.values) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        [&]{ std::vector<int64_t> v; for (auto x : config.tile_shape.block_tile.d.values) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        [&]{ std::vector<int64_t> v; for (auto x : config.tile_shape.block_tile.dv.values) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
