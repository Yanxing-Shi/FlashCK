#include "core/profiling/fmha/fmha_fwd_append_kv/fmha_fwd_append_kv_emitter.h"
...
std::vector<FmhaFwdAppendKVCodeGen> FmhaFwdAppendKVEmitter::CreateInstanceForConfig(const FmhaFwdAppendKVConfig& config, const FmhaProblem& fmha_problem) {
    ...
    std::vector<std::vector<int64_t>> all_lists = {
        // BlockConfig
        [&]{ std::vector<int64_t> v; for (auto x : config.tile.block.s.values) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        [&]{ std::vector<int64_t> v; for (auto x : config.tile.block.sk.values) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        [&]{ std::vector<int64_t> v; for (auto x : config.tile.block.d.values) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
        [&]{ std::vector<int64_t> v; for (auto x : config.tile.block.dv.values) v.emplace_back(static_cast<int64_t>(x)); return v; }(),
...
void FmhaFwdAppendKVEmitter::GenerateInstances(FmhaProblem& fmha_problem)
{
    ...
    for (const auto& config : g_backup_fmha_append_kv_config) {
        ...
        fmha.tile_desc_ = FmhaFwdAppendKVTileDesc{
            config.tile.block.s.values[0],
            config.tile.block.sk.values[0],
            config.tile.block.d.values[0],
            config.tile.block.dv.values[0]
        };
