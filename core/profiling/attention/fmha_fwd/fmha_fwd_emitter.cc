#include "core/profiling/attention/fmha_fwd/fmha_fwd_emitter.h"
...
bool FmhaFwdEmitter::IsValidTile(const FmhaFwdTileDesc& tile_desc, const FmhaFwdProblem& fmha_fwd_problem)
{
...
bool FmhaFwdEmitter::IsValidInstance(const FmhaFwdCodeGen& instance)
{
    return IsValidTile(instance.tile_desc_, instance.problem_);
}

std::vector<FmhaFwdCodeGen> FmhaFwdEmitter::HeuristicFilter(
    const std::vector<FmhaFwdCodeGen>& instances,
    const FmhaFwdProblem& fmha_fwd_problem) const
{
    if (instances.empty()) {
        return {};
    }

    std::vector<FmhaFwdCodeGen> filtered;
    ...
    const int64_t seq_len_ratio = fmha_fwd_problem.q_seq_len_ / fmha_fwd_problem.kv_seq_len_;
    ...
    for (const auto& instance : instances) {
        ...
        if (tile.k0_block_ < fmha_fwd_problem.qk_head_dim_ / 4) {
            continue;
        }
...
std::vector<FmhaFwdCodeGen> FmhaFwdEmitter::CreateInstanceForConfig(
    const FmhaFwdConfig& config, const FmhaFwdProblem& fmha_fwd_problem) 
{
    ...
    CartesianProduct(all_param_lists, [&](const std::vector<int64_t>& param_values) {
        ...
        // Construct FmhaFwdCodeGen instance
        FmhaFwdCodeGen instance;
        instance.problem_ = fmha_fwd_problem;
...
void FmhaFwdEmitter::GenerateInstances(FmhaFwdProblem& fmha_fwd_problem)
{
    ...
    auto base_json_path = std::filesystem::path(FLAGS_FC_CONFIG_JSON_PATH) / "attention" / "fmha_fwd";
    std::vector<FmhaFwdCodeGen> all_instances;
    ...
    if (FLAGS_FC_ENABLE_BACKUP_JSON) {
        ...
        try {
            auto backup_configs = LoadConfigJson<std::vector<FmhaFwdConfig>>(json_path);
            for (const auto& config : backup_configs) {
                auto instances = CreateInstanceForConfig(config, fmha_fwd_problem);
    ...
    if (FLAGS_FC_ENABLE_DEFAULT_JSON) {
        ...
        try {
            auto default_config = LoadConfigJson<FmhaFwdConfig>(json_path);
            auto instances = CreateInstanceForConfig(default_config, fmha_fwd_problem);
            all_instances.insert(all_instances.end(), instances.begin(), instances.end());
    ...
    if (FLAGS_FC_ENABLE_USER_JSON) {
        ...
        try {
            auto user_config = LoadConfigJson<FmhaFwdConfig>(json_path);
            auto instances = CreateInstanceForConfig(user_config, fmha_fwd_problem);
            all_instances.insert(all_instances.end(), instances.begin(), instances.end());
    ...
    std::vector<FmhaFwdCodeGen> valid_instances;
    ...
    std::vector<FmhaFwdCodeGen> final_instances;
    switch (FLAGS_FC_TUNING_MODE) {
        case 0: {
            // Heuristic mode: filter + random selection
            final_instances = HeuristicFilter(valid_instances, fmha_fwd_problem);
        ...
        case 2: {
            ...
            auto heuristic_instances = HeuristicFilter(valid_instances, fmha_fwd_problem);
...
void FmhaFwdEmitter::ClearInstances()
{
    instance_map_.clear();
    num_instances_ = 0;
