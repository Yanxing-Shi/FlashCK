...
#include "ater/core/profiler/codegen.h"
...
// kernel tunning
void Context::CodegenAndProfileKernel()
{
    // step1: profile kernel
    // using FLAGS_selected_gpus to Get devices
    std::vector<int> selected_gpu = Target::Instance()->GetTargetSelectedDevices();
    LOG(INFO) << "Selected devices for profiling: " << selected_gpu;

    std::vector<std::vector<std::tuple<std::filesystem::path, std::filesystem::path>>> graph_generated_profilers =
        GenProfiler(model_ops_);

    std::vector<Operation*> gemm_ops = model_ops_;

    // Gemm
    Builder gemm_builder;
    gemm_builder.MakeProfilers(graph_generated_profilers, context_name_, true);
    auto gemm_postprocess_ptr = std::make_shared<GemmProfilerPostprocessingDelegate>();
    auto profiler_runner_ptr  = std::make_shared<GPUProfilerRunner>(selected_gpu.size(), gemm_postprocess_ptr, 100000);
    for (Operation* gemm_ptr : gemm_ops) {
        gemm_ptr->Profile(profiler_runner_ptr);
    }
    profiler_runner_ptr->Join();

    // step2: gen function src
    std::vector<std::tuple<std::filesystem::path, std::filesystem::path>> file_tuples =
        GenFunctionSource(model_ops_, context_name_);
    gemm_builder.MakeProfilers({file_tuples}, context_name_, false);
}
...
void Context::BuildContext()
{
    ...
    LOG(INFO) << "start ater context build, Mode: " << ModeToStr(mode_);
    ...
    for (Layer* root_layer : root_layers_) {
        LOG(INFO) << "context start build layer: " << root_layer->GetName();
        root_layer->Forward();
    }

    LOG(INFO) << "Context has build layer, Mode: " << ModeToStr(mode_);
    ...
    try {
        ...
        ATER_THROW(ResourceExhausted("free temporary buffer {} failed!", tmp_buff_));
    }

    mem_manager_ptr_->CalculateBuffer();

    is_context_built_    = true;
    is_context_building_ = false;

    Synchronize();

    LOG(INFO) << "Finish context build success, Mode: " << ModeToStr(mode_);
}

int Context::CreateGlobalContext(const std::string& model_name, Mode mode, const int device_id)
{
    global_context_id_++;
    std::shared_ptr<Context> context_ptr = std::make_shared<Context>(model_name, mode, device_id);
    global_context_ptr_                  = context_ptr;
    global_contexts_map_.emplace(global_context_id_, context_ptr);
    LOG(INFO) << "create global context success, context_id: " << global_context_id_;
    return global_context_id_;
}

void Context::SetGlobalContext(const int context_id)
{
    auto iter = global_contexts_map_.find(context_id);
    if (iter == global_contexts_map_.end()) {
        LOG(ERROR) << "Error occured! context_id " << context_id << " does not exist!";
        exit(-1);
    }
...
void Context::UpdateBeginRegressIdx(int node_idx)
{
    ...
    beigin_regress_idx_ = (beigin_regress_idx_ == -1) ? node_idx : std::min(node_idx, beigin_regress_idx_);
}
...
std::unordered_map<int, std::shared_ptr<Context>> Context::global_contexts_map_ = {};
