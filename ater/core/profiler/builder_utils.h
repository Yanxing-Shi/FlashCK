...
ATER_DECLARE_bool(ATER_TIME_COMPILATION);
...
{
    return FLAGS_ATER_TIME_COMPILATION ? Sprintf("time -f 'exit_status=%x elapsed_sec=%e argv=\"%C\"' {}", cmd) : cmd;
}
...
{
    LOG(INFO) << "Running make commands: " << JoinToString(cmds);
    ...
    if (!cached_results_available) {
        ...
        try {
            ...
            LOG(INFO) << "make stdout: " << stdout_str;
            LOG(WARNING) << "make stderr: " << stderr_str;
        }
