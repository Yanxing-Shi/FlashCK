...
ATER_DECLARE_string(selected_gpus);
...
int GetGPUDeviceCount()
{
    ...
    int device_count = 0;
    ATER_ENFORCE_HIP_SUCCESS(hipGetDeviceCount(&device_count));
    return device_count;
}
...
int GetGPUComputeCapability(int id)
{
    ...
    ATER_ENFORCE_HIP_SUCCESS(major_error_code);
    ATER_ENFORCE_HIP_SUCCESS(minor_error_code);
...
int GetGPURuntimeVersion(int id)
{
    ...
    ATER_ENFORCE_HIP_SUCCESS(hipRuntimeGetVersion(&runtime_version));
...
int GetGPUDriverVersion(int id)
{
    ...
    ATER_ENFORCE_HIP_SUCCESS(hipDriverGetVersion(&driver_version));
...
int GetGPUMultiProcessors(int id)
{
    ATER_ENFORCE_LT(id,
                    ...
                    InvalidArgument("Device id must be less than GPU count, "
                                    ...
                                    GetGPUDeviceCount()));
    int count;
    ATER_ENFORCE_HIP_SUCCESS(hipDeviceGetAttribute(&count, hipDeviceAttributeMultiprocessorCount, id));
    return count;
}
...
int GetGPUMaxThreadsPerMultiProcessor(int id)
{
    ATER_ENFORCE_LT(id,
                    ...
                    InvalidArgument("Device id must be less than GPU count, "
                                    ...
                                    GetGPUDeviceCount()));
    int count;
    ATER_ENFORCE_HIP_SUCCESS(hipDeviceGetAttribute(&count, hipDeviceAttributeMaxThreadsPerMultiProcessor, id));
...
int GetGPUMaxThreadsPerBlock(int id)
{
    ATER_ENFORCE_LT(id,
                    ...
                    InvalidArgument("Device id must be less than GPU count, "
                                    ...
                                    GetGPUDeviceCount()));
    int count;
    ATER_ENFORCE_HIP_SUCCESS(hipDeviceGetAttribute(&count, hipDeviceAttributeMaxThreadsPerBlock, id));
    return count;
}
...
const hipDeviceProp_t& GetDeviceProperties(int id)
{
    ...
    std::call_once(*(g_device_props_init_flags[id]),
                   [&] { ATER_ENFORCE_HIP_SUCCESS(hipGetDeviceProperties(&g_device_props[id], id)); });
...
{
    int device_id;
    ATER_ENFORCE_HIP_SUCCESS(hipGetDevice(&device_id));
    return device_id;
}

void SetDeviceId(int device_id)
{
    ATER_ENFORCE_LT(device_id,
                    GetGPUDeviceCount(),
                    InvalidArgument("Device id must be less than GPU count, "
                                    "but received id is: {}. GPU count is: {}.",
                                    device_id,
                                    GetGPUDeviceCount()));
    ATER_RETRY_HIP_SUCCESS(hipSetDevice(device_id));
}
...
void GetSetDevice(int input_device_id, int* output_device_id)
{
    if (output_device_id != nullptr) {
        ...
        ATER_ENFORCE_HIP_SUCCESS(hipGetDevice(&curr_device_id));
...
{
    ATER_ENFORCE_HIP_SUCCESS(hipStreamSynchronize(stream));
}

void GpuDestroyStream(hipStream_t stream)
{
    ATER_ENFORCE_HIP_SUCCESS(hipStreamDestroy(stream));
}
